...
CSPHParticle::CSPHParticle(const Vector3df& position, const float radius, CSPHFluid* fluid) :
	position(position),
	radius(radius),
	fluid(fluid)
{
...
{
	return density / fluid->getDensity();
}

float CSPHParticle::getPressure() const
{
	return fluid->getPressureCoe() * (std::pow(getDensityRatio(), 1.0f) - 1.0f);
}

float CSPHParticle::getMass() const
{
	return fluid->getDensity() * std::pow(getDiameter(), 3.0f);
}
...
{
	return getMass() / fluid->getDensity();
}
...
void CSPHParticle::solveSurfaceTension(const CSPHParticle& rhs)
{
	...
	const float tensionCoe = (this->fluid->getTensionCoe() + rhs.fluid->getTensionCoe()) * 0.5f;;
...
{
	const auto viscosityCoe = (this->fluid->getViscosityCoe() + rhs.fluid->getViscosityCoe()) * 0.5f;
