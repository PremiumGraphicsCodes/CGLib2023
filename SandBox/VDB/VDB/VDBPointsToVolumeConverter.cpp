#include "VDBPointsToVolumeConverter.h"
...
std::vector<VDBVolume*> SmoothVolumeConverter::build(VDBPoints* vdbParticles, const float particleRadius, const float cellLength, const std::vector<std::string>& floatNames)
{
	std::vector<VDBVolume*> vdbVolumes;

	auto vdbVolume = new VDBVolume();
